...
public class TableMergeIndex {
	...
	/*
	 * index第一次简单自去重,通过set来完成 index_name,indexed_columns两者都相同,才会去重
	 */
	...
	/*
	 * index第二次去重 如果set_new_indexes中的index,已在set_exist_indexes存在,那么需要删除
	...
	/*
	 * index第三次去重 具有相同的索引字段,索引名不同,这样的索引只需要保留一个即可 这种情况只存在于两个set之间,不可能存在于单个set内部
	 */
	...
	/*
	 * 同一个表上的索引名不能相同 需要处理set_result_new_indexes的索引名
	 */
	...
	/*
	 * 对原来存在的索引进行自合并 drop index index_name on table
	 */
	...
	/*
	 * 删掉在set_new_indexes与老的索引字段相似的节点 这里必须要按字段比较
	 */
