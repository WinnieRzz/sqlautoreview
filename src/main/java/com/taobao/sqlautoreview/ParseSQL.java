...
public class ParseSQL {
	...
	private void selectColumnCheckValid(String columnsString) {
		if (columnsString.equals("*") == true) tip = "出现了select *,这是不倡导的,请写出明确的column name.";
	}
	...
	/*
	 * 淘宝mysql单表分页的规范 按照分页的规则来匹配,处理单表分页的解析 示例1 root@test 09:44:03>explain select
	 * t1.id, t1.manager_nick, t1.gmt_create, t1.nick -> from (select id -> from
	 * test -> where manager = '' -> and is_open = 2 -> order by gmt_create
	 * limit 1, 10) t2 straight_join test t1 -> where t1.id=t2.id 示例2 root@test
	...
	private void parseSQLSelectPage() {
		...
		// select primary key from table name where 条件 order by column_name
		// asc/desc limit #1,#2
